---
title: "Multiple imputation of multi-domain items, confounders and pain outcomes"
...
Using sample of `r nrow(dfkeep)` participants with data on pain status at ages 26 and 30. 

## Set up

```{r mult-imp-matrix, message=FALSE, warning=FALSE, out.width='100%'}
...
```{r manual-pred-matrix, out.width='100%'}
imp_pred_mat_plt <- plot_pred(predm, rotate = T, label = F, method = imp_methods) +
  theme(text = element_text(size=2), 
        axis.text.x = element_text(size=2.5, hjust=0, vjust = -0.01), 
...
tile_layer <- which(sapply(pred_mat_plt$layers, function(x) class(x$geom)[1]) == "GeomTile")
pred_mat_plt$layers[[tile_layer]]$aes_params$linewidth <- 0.02

ggsave(plot=imp_pred_mat_plt, filename='imp_pred_mat_plt.png',path="//path/to/outputs/", width=194, height=154, units="mm", dpi=350)
```
